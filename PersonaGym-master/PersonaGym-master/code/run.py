...
SETTINGS_MODEL = os.getenv("PERSONAGYM_SETTINGS_MODEL", DEFAULT_OPENAI_MODEL)
QUESTION_MODEL = os.getenv("PERSONAGYM_QUESTION_MODEL", DEFAULT_OPENAI_MODEL)
EXAMPLE_MODEL = os.getenv("PERSONAGYM_EXAMPLE_MODEL", DEFAULT_OPENAI_MODEL)
EVAL_1 = os.getenv("PERSONAGYM_EVAL_MODEL", DEFAULT_OPENAI_MODEL)
...
def gen_score_examples(persona, qa, rubric, model):
    ...
    if not isinstance(examples, str) or examples.strip() in {"", "Error"}:
        raise RuntimeError(f"Failed to generate score examples with model {model}: {examples!r}")

    rubric_blocks = parse_full_examples(examples)
    if len(rubric_blocks) != len(qa):
        raise RuntimeError(
            f"Expected {len(qa)} rubric example blocks, got {len(rubric_blocks)}. Raw response starts with: {examples[:300]!r}"
        )

    return [process_examples(block) for block in rubric_blocks]
...
def format_rubrics(persona, rubric, qa):
    ...
    for i in range(len(qa)):
        ...
        formatted_rubric = rubric.format(
            ...
            question=question,
            response=answer,
            score_example="No score examples provided. Evaluate directly from the rubric criteria and justify the score carefully.",
        )
